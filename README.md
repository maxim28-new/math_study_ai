...
界面已经做成手机网页。孩子打开后只看到聊天；主题、难度、密钥都收在右上角齿轮里，给家长改。

### 家里最快的用法（推荐）

电脑填好 `.env` 后启动，同一 WiFi 下手机直接打开：
...
手机打开（同一 WiFi）： http://192.168.x.x:8000
```
...
仓库打开 **Settings → Pages**，Source 选 **GitHub Actions**（本仓库已带部署工作流）。地址形如 `https://<用户名>.github.io/math_study_ai/`。
...
### 🧭 一起探索（小欧出题）
选一个**主题**，小欧就从这个主题的几条**公理**出发，给孩子出一道值得琢磨的探索题，陪他一步步**自己发现**一个规律或道理。探索模式点「开始玩」，立刻出现可拖的方块盘；一个探索告一段落还能在「需要帮忙」里「换一题」。
...
> **每次出的题是写死的吗？** 不是。各主题只提供几条"出题灵感"作为**种子**，具体题目的场景、数字、顺序都是模型**每次现场生成**的（并已指示它刻意变换花样），所以同一主题反复点「开始玩 / 换一题」会得到不同的题。想更换风格，改 `server/tutor.py` 里各主题的 `inspirations` 即可。
...
- **语音输入**：点一下麦克风开始说话，再点一下结束；结束后把完整识别结果写进输入框（用浏览器自带的语音识别，Chrome/Edge/Safari 支持；不支持时按钮会置灰）。
- **画板输入**：点画板按钮，手写或画个图，"发送给小欧"——它会作为一张图走拍照读题的同一条通道。
- **拍照输入**：带题模式下拍作业本上的题。

## 思考模式（更快 or 更深）

右上角齿轮（家长设置）里可切换**思考模式**，并可单独选择是否展示思考过程：
- **关闭**（默认）：回答更快，适合日常陪练。
- **开启**：模型先深度推理再回答，适合难题/奥数。
- **展示思考过程**：可独立关闭。给孩子用时建议关闭，只看最终回复；家长调试时再打开灰色思考块。
...
若走上面的「没有电脑时：纯网页版」，家长在齿轮里填密钥即可：

- **文字模型（必填）**：推荐 **DeepSeek**。接口 `https://api.deepseek.com/v1`，模型 `deepseek-v4-flash`，密钥去 [platform.deepseek.com](https://platform.deepseek.com) 申请。
- **视觉模型（选填，拍照读题）**：推荐魔搭 ModelScope Qwen-VL。不填就先用文字功能。

> 浏览器有跨域限制。实测 **DeepSeek** 和 **魔搭 ModelScope** 允许网页直连；通义百炼、Kimi、智谱默认不行（那些请用上面的「家里最快的用法」，密钥放电脑 `.env`）。

---

## 它长什么样

- 顶部：**一起探索 / 带题来问**，右上角齿轮是家长设置（名字、主题、难度、思考开关）。
