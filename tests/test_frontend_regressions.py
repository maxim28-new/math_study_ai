...
class FrontendRegressionTests(unittest.TestCase):
    ...
    def test_web_voice_uses_tap_to_talk_dock(self):
        ...
        self.assertIn("hold-speak-btn", html)
        self.assertIn(".hold-speak-btn", css)
        self.assertIn("function initVoice(", app)
        self.assertIn("function bindHoldSpeak(", app)
    ...
    def test_explore_stage_shell(self):
        ...
        self.assertIn(".app.layout-explore:not(.draft-open) #inputRow", css)
        self.assertIn(".app.layout-explore:not(.can-speak) #voiceDock { display: none; }", css)
        self.assertIn(".hold-speak-btn", css)
        ...
        self.assertIn("function setDraftOpen(", app)
        ...
        self.assertIn(".explore-stage.is-drawing .play-stage { flex: 1 1 0; }", css)
        ...
        self.assertIn("function syncVoiceClearButton(", app)
        self.assertIn("清空", html)
        self.assertIn(".voice-row", css)
        self.assertIn(".voice-clear-btn", css)
        self.assertNotIn('class="voice-clear-btn hidden"', html)
        self.assertNotIn("id=\"voiceHint\"", html)
        self.assertNotIn("说错了，撤销刚才", html)
        self.assertIn("太难了", html)
        self.assertIn('id="easierBtn"', html)
        self.assertIn("function askForEasier(", app)
        self.assertNotIn("再小一点", html)
        ...
        self.assertIn("from_sense_voice", read("server/asr.py"))
        self.assertIn("LLM_ASR_MODEL=local", read(".env.example"))
        self.assertIn("LLM_ASR_LOCAL_ENGINE=auto", read(".env.example"))
        ...
        self.assertNotIn('id="talkBtn"', html)
    ...
    def test_board_v3_assets_and_renderers(self):
        ...
        caption = html.split('id="captionBar"', 1)[1].split("</section>", 1)[0]
        self.assertNotIn('id="termCard"', caption)
        self.assertIn('id="tutorCaption"', caption)
        self.assertIn("按住说话", caption)
    ...
    def test_explore_refresh_restores_last_caption(self):
        ...
        self.assertIn("function liveCaption(", app)
        self.assertIn("function isIdleCaption(", app)
        self.assertIn("function messagePlainText(", app)
        self.assertIn("caption: liveCaption()", app)
        hist = app[app.find("function renderHistory"):app.find("async function loadConfig")]
        self.assertIn("restoreExploreCaption()", hist)
        self.assertIn("mountFromCard(card)", hist)
        self.assertIn("topicCard(state.problemCard)", hist)
        ...
        self.assertIn("liveCaption()", restore)
        self.assertIn("START_CAPTION", app)
        self.assertIn("function clearStageHost(", app)
